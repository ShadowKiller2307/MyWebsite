...
function distance_point_point(p1: point, p2: point) {
    return Math.sqrt(Math.pow(p1.position.x - p2.position.x, 2) + Math.pow(p1.position.y - p2.position.y, 2))
}

function distance_position_position(p1: vector, p2: vector) {
    return Math.sqrt(Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2))
}


//tests if two objects collide
function collide_point_point(p1: point, p2: point) {
...
function collide_point_point_Radius(p1: point, p2: point) {
...
function collide_point_circle(p1: point, c1: circle) {
...
function collide_circle_circle(c1: circle, c2: circle) {
...
function collide_point_rectangle(p1: point, r1: rectangle, drawPoints?: boolean) {
    let angle = r1.angle;
...
function collide_circle_rectangle(c1: circle, r1: rectangle, drawPoints?: boolean) {
    let angle = r1.angle,
...
function collide_rectangle_rectangle(r1: rectangle, r2: rectangle, drawPoints?: boolean) {
...
function rotate_e1_around_Pos_e2(e1: element, e2: element, angle: number) {
...
function rotate_e1_around_Pos_e2_Return(e1: rectangle | circle | point, e2: rectangle | circle | point, angle: number) {
    let c1X = Math.cos(angle) * (e1.position.x - e2.position.x) - Math.sin(angle) * (e1.position.y - e2.position.y) + e2.position.x;
    let c1Y = Math.sin(angle) * (e1.position.x - e2.position.x) + Math.cos(angle) * (e1.position.y - e2.position.y) + e2.position.y;
    let object: any = e1;
